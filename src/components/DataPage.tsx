...
export default function DataPage({ data }: { data: IProps[] }) {
  const selectDeselect = () => {
    let selectAllEl = document.getElementById('select-all') as HTMLInputElement;
    selectAllEl?.addEventListener('click', () => {
      let boxes: NodeListOf<HTMLElement> = document.getElementsByName('checkbox');
      // converts to an actual array
      let boxesArray: HTMLInputElement[] = Array.prototype.slice.call(boxes)
      if (selectAllEl.checked === true) {
        for (let i = 0; i < boxesArray.length; i++) {
          if (boxesArray[i].type === 'checkbox') boxesArray[i].checked = true;
        }
      } else {
        for (let i = 0; i < boxes.length; i++) {
          if (boxesArray[i].type === 'checkbox') boxesArray[i].checked = false;
        }
      }
    });
  };
